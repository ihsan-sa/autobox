...
- **A gate or a build failing with `Disk quota exceeded` while `df` shows room?** The limit that ran out is this user's own on the temp root, and `df` cannot see it: `cc-janitor quota` prints it (`box-status` shows the same line). The daily janitor clears leftovers nothing has written to for `CC_JANITOR_SCRATCH_HOURS` (default 6 h) and drops to `CC_JANITOR_SCRATCH_PRESSURE_HOURS` (1 h) once that usage passes `CC_JANITOR_SCRATCH_PRESSURE` (85%); `cc-janitor --configured --dry` shows what it would take. Nothing a process is standing in, holding open or running out of is ever a candidate, nor a directory a `CC_JANITOR_HOLDS` record points into, nor a harness session's scratch tree while that session is still writing its transcript.

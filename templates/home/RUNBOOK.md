...
- **One actor per unit for a restart go.** An owner "go" for a service restart belongs to whichever seat asked for it or whichever landing already carries it — not to every seat watching the same event. Before restarting a unit a landing might already have touched, check `systemctl --user show <unit> -p ActiveEnterTimestamp --value` for a start time newer than the go; if it is, the restart already happened and a second one is a duplicate, not a confirmation.

- **A gate or a build failing with `Disk quota exceeded` while `df` shows room?** The limit that ran out is this user's own on the temp root, and `df` cannot see it. There is no automatic cleanup for it any more — `cc-janitor`'s scratch-disk rules and `quota` are gone — so check it by hand and clear it yourself. Sessions and the box's units keep their scratch in `~/.cc/tmp` on disk (`TMPDIR`, set by `cc`, `cc-loop` and `cc-rc` when nothing else has), so what is left in `/tmp` is what something started outside them wrote. A landing's checkouts and gate logs live under one `/tmp/cc-land.run.<pid>.*` root that the run removes when it ends, keeping only a red gate's log; the next landing worker removes any such root whose pid is dead.

...
- **One command lands it: `cc-land <repo> <pr-number>`.** Gates, merge, the `#approvals` card marked 👌, board, ask ledger, `install.sh`, switching on any unit the change *added*, and restarting only what moved — in that order, one line each, stopping at the first real failure. Work with no PR (a config edit, a host change) closes the same way: `cc-land <repo> --track <t> --note "<what changed>"`. It never merges on a promise (gates run on the PR's own head, in a throwaway worktree), never switches on a unit that is off for a reason, and never restarts the box's tmux server. Re-running is the recovery and is safe. Workers cannot call it — `cc-guard` denies it exactly as it denies `gh pr merge` and `systemctl`.
...
## Cheat sheet
    ...
    cc-land myapp 71                     land it: gates, merge, card, board, ledger, install, units, restarts

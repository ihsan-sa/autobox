...
- Each finished track's PR is one card in `#approvals`; 👍 queues a landing job — the gates against the PR's own head, then a review pass, then merge, install and restart — with the outcome in the thread. A failed merge pushes to your phone, posts in `#<repo>-updates` and is injected into the repo's session.

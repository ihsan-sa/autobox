...
- Channel membership is the access control: anyone in a channel that routes to a session is answered as `role="member"`. A member cannot authorize an owner gate, answer a 🔐 prompt, run a `!` command, or hand a thread over; the session does the ungated part and hands the rest to the planning seat (`cc-notify`, or `reply(needs_owner)`, which files a request), and that seat asks you if only you can decide. A member's 👍 on a card in `#approvals` queues that PR for landing (repos named in `CC_SELF_LAND_EXCEPT`); a PR on a protected path is queued only by your own 👍 on its 🔐 card in `#approvals`; every other repo lands its own green PRs. A non-owner DM gets 👋 and one line. Each non-owner also gets a private `#<handle>` pair and a `~/dev/<handle>` workspace on a daily budget (`cc-slack member add`).
...
- Commands that need no session: `!pause` `!resume` `!restart box|<repo>[/track]|slack|tmux` `!reboot` `!handoff [<repo>[/track]]` (`!handoff` hands the channel's session off to a fresh one, which reloads its skills; `!restart tmux` and `!reboot` confirm first — RUNBOOK). Anything else you type is a message to the session.
- Unasked: `cc-notify` into `#<repo>-updates` (`--decision` climbs to `#<repo>` with a mention); boot and audit lines into `#alerts`; a worker loop posts only its DONE card or one BLOCKED question, and iterations, budgets, handoffs, model switches, joins and pause fan-out stay in their logs; the daily digest is no longer posted by itself: it stays on the Home tab and the boards, and `cc-reconcile digest --notify` posts it when asked; an escalation (`--owner`) from the planning seat is a DM and a phone push; from a member-facing session or a member workspace it is a request to the planning seat, never your DM and never the channel it came from. `#approvals` holds only what needs your OK: a repo that lands itself keeps its PR cards in its `-updates` lane. A usage limit pauses workers and shows as `⏳ Claude usage limit until …`. `cc-audit` (03:30 UTC) puts its report on the control repo's canvas and its findings on the board (`audit-`/`arch-`/`delete-…` rows); nothing is fixed until you say so.

...
%
% THERE ARE NO FIGURES IN THIS FILE, AND THERE MUST NOT BE. The last page is counted when the PDF is built —
% cc-arch reads the spend ledger and the landing queue and writes `arch-figures.tex` beside this file, which
% the last line of the document \input's. A number typed in here is right for one day and is then rebuilt,
% unchanged and believed, on every revision after it; that is what happened to the money page of 2026-09-06.
% This file still builds by hand: with no `arch-figures.tex` next to it, the last page says so.
%
% It lived in a session scratchpad for its first four versions, which meant every update began by finding
% it. Keep it here. When the box changes shape, this file is the thing to update — and say so in a commit
% message on the branch, in the commit BODY (never the subject), at the start of a line of its own:
%
% ARCHITECTURE: <one line — what about the shape of the box is different now>
%
% cc-arch reads that line off the squash commit after the change lands, rebuilds this file and puts the PDF in
% the channel. Nothing else triggers it: a change that rewrites this document and declares nothing sends
% nothing. There is no dated copy of this file to keep in step — the dated thing is the PDF, built from here.
...
\vspace{0.9em}
{\small Nothing counted the money or the time for this copy. The figures belong to the moment a PDF is made:
they are read from the spend ledger and the landing queue then, and they are deliberately not in this source.
Build it with \cmd{cc-arch build} to get them.}}

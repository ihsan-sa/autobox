% autobox-overview.tex — the illustrated overview: the cycle from a message to a merge, how the work is
% shaped, and a last page of figures. Written for someone who has not seen the box before.
%
...
% and texlive-pictures. Check the result with `pdfinfo` (pages) and `pdffonts` (all embedded).
%
% THERE ARE NO FIGURES IN THIS FILE, AND THERE MUST NOT BE. A number typed in here is right for one day and is
% then reprinted, unchanged and believed, on every revision after it; that is what happened to the money page of
% 2026-09-06. The figures belong to whatever counts them at the moment a PDF is made: the last page \input's an
% `arch-figures.tex` sitting beside this file if there is one. A hand build — which is every build — has none,
% and that page says so rather than printing figures nobody counted.
%
% It lived in a session scratchpad for its first four versions, which meant every update began by finding
% it. Keep it here. NOTHING REBUILDS IT AND NOTHING POSTS IT: when the box changes shape, whoever changes it
% redraws this file in the same change and builds the PDF by hand with the line above. There is no dated copy
% of this file to keep in step — the dated thing is the PDF, built from here.
...
\usepackage{enumitem}
\usepackage{booktabs}   % the built-at-build-time page's tables; nothing outside texlive-latex-recommended
...
\node[box,text width=3.2cm] (p1) at (3.75,0)
  {\tcmd{cc-context} reads the context a session has used\\[1pt] one line: 150k tokens (\tcmd{CC\_CONTEXT\_HANDOFF\_TOKENS})};
\node[box,text width=2.75cm] (p2) at (7.0,0)
  {past the line: finishes what is in flight, then a successor is started};
...
Crossing the line restarts nothing by itself. The successor starts at the first turn with nothing
...
% ==========================================================================
% PAGE 3 — where the cost goes. Written at build time, not here: see the head of this file.
% ==========================================================================
\InputIfFileExists{arch-figures}{}{%
\begingroup\raggedright
{\sffamily\bfseries\Large Where the cost goes}\\[0.3em]
{\color{line}\rule{\linewidth}{1pt}}
\endgroup

\vspace{0.9em}
{\small Nothing counted the money or the time for this copy, and this page is built by hand. The figures belong
to the moment a PDF is made — they are read from the spend ledger and the landing queue then — and they are
deliberately not in this source, where a number is right for a day and then reprinted for months.}}

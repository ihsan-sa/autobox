...
4. From here `cc-reconcile`'s tick wakes a planning seat when a queued row can start or one has stopped, and it works the loop in `WORKING.md` on its own; what still needs the owner is the approval list in `~/CLAUDE.md`, and it reaches them by @-mention.
